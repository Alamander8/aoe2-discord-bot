...
class SpectatorCore:
    ...
    def calculate_minimap_mask(self, minimap_image):
        """Calculate the playable area mask of the minimap."""
        try:
            ...
            minimap_contour = max(contours, key=cv2.contourArea)
            mask = np.zeros_like(gray)
            cv2.drawContours(mask, [minimap_contour], -1, (255), -1)
            
            # Store last valid mask
            self.last_minimap_mask = mask
    ...
    def detect_activity_zones(self, minimap_image):
        """Detect activity zones on the minimap using color detection."""
        activity_zones = []
